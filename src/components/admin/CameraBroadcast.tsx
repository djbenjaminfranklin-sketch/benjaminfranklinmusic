...
import { Video, VideoOff, Eye, Mic, MicOff, UserPlus, UserMinus, Camera, SwitchCamera, Minimize2, X } from "lucide-react";
  ...
  if (isBroadcasting && localStream && isFullscreen) {
    return (
      <div className="fixed inset-0 bg-black z-50">
        ...
        {/* Layer 1: Chat overlay — between video and controls, pointer-events only on the form */}
        {chatMessages && onSendChat && (
          <div className="absolute left-0 right-0 top-24 bottom-32 z-10 pointer-events-none">
            <LiveChatOverlay messages={chatMessages} onSend={onSendChat} />
          </div>
        )}

        {/* Layer 2: Top bar — ABOVE chat */}
        <div className="absolute top-0 left-0 right-0 z-30 pointer-events-none">
          <div className="p-4 pt-[max(3.5rem,calc(env(safe-area-inset-top)+1rem))] flex items-start justify-between pointer-events-auto">
            {/* Left: LIVE badge */}
          ...
          {allStreams.length > 1 && (
            <div className="flex items-center justify-center gap-1.5 mt-1 pointer-events-auto">
              {allStreams.map((s, i) => (
                <button
                  key={s.id}
        ...
        {/* Guest notification */}
        {guestNotification && (
          <div className="absolute top-28 left-1/2 -translate-x-1/2 z-40 rounded-full bg-accent/90 backdrop-blur-sm px-4 py-2 animate-bounce">
            <span className="text-sm font-bold text-background">{guestNotification}</span>
          </div>
        )}

        {/* Active guests — eject buttons (right side, middle) */}
        {coHostEntries.length > 0 && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-2">
            {coHostEntries.map(([id]) => (
              <button
                key={id}
                onClick={() => disconnectGuest(id)}
                className="flex items-center gap-1.5 rounded-full bg-red-500/80 backdrop-blur-sm px-2.5 py-1.5 border border-red-500/50 active:scale-95 transition-transform"
              >
                <UserMinus className="h-3.5 w-3.5 text-white" />
                <span className="text-[10px] font-bold text-white">{guestNames.get(id) || id.slice(0, 6)}</span>
        ...
        {/* Layer 3: Bottom controls — ABOVE everything */}
        <div className="absolute bottom-0 left-0 right-0 z-30 pointer-events-none">
          <div className="p-6 pb-[max(2rem,calc(env(safe-area-inset-bottom)+0.5rem))] flex items-center justify-center gap-3 bg-gradient-to-t from-black/80 to-transparent pointer-events-auto">
            {/* Mute */}
            <button
              onClick={toggleMute}
              className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center active:scale-95 transition-transform"
            >
              {isMuted ? (
            ...
            {!isCoHost && (
              <button
                ...
                disabled={inviting}
                className="w-12 h-12 rounded-full bg-accent/20 backdrop-blur-sm border border-accent/30 flex items-center justify-center active:scale-95 transition-transform disabled:opacity-50"
              >
