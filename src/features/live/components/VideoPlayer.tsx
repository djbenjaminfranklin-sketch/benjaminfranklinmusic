...
export default function VideoPlayer({ src, stream, streamType }: VideoPlayerProps) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Mute/unmute toggle — visible after initial unmute overlay is dismissed */}
      {!isMutedOverlay && !isLoading && (
        <button
          onClick={toggleMute}
          className="absolute bottom-3 right-3 z-10 w-10 h-10 rounded-full bg-black/60 backdrop-blur-sm border border-white/10 flex items-center justify-center active:scale-95 transition-transform"
        >
          {isMuted ? (
